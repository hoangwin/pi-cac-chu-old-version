...
public class Map
{
	...
	public void NewGame()
	{
		...
		countAutoSort = 1;
		mAllScore = 0;
		mTimerDecrease = 120000 - NoiTraiCay.mcurrentlevel * 500;
		mTimerCount = 0;
		if (effect1 == null) {
			effect1 = new Actor();
			effect2 = new Actor();
	...
	public static void DrawPath(Canvas g)
	{
		...
		if (countFrameDrawPath-- >= 0) {
			...
			NoiTraiCay.mainPaint.setStyle(Style.STROKE);
			NoiTraiCay.mainPaint.setStrokeWidth(5);
			NoiTraiCay.mainPaint.setColor(Color.BLUE);
			...
			for (i = 1; i < rCount; i++) {
				...
				g.drawLine(x1, y1, x2, y2, NoiTraiCay.mainPaint); // Váº½ Ä‘Æ°á»�ng tháº³ng ná»‘i 2 Ã´
				x1 = x2;
				y1 = y2;
		...
		if (countFrameDrawAddScrore-- >= 0) {
			...
			NoiTraiCay.mainCanvas.drawText(" + " + mScoreAdd ,x1, y1 - (20 - countFrameDrawAddScrore * 2), NoiTraiCay.android_FontYellow);
	...
	public static void DrawGame(Canvas g)
	{
		...
		NoiTraiCay.mainPaint.setColor(Color.BLUE);
		int SquareFrame;
		if (NoiTraiCay.mcurrentlevel < 15) {
			SquareFrame = 20;
		} else if (NoiTraiCay.mcurrentlevel < 30) {
			SquareFrame = 22;
		} else if (NoiTraiCay.mcurrentlevel < 45) {
			SquareFrame = 24;
		} else {
			SquareFrame = 26;
		}
