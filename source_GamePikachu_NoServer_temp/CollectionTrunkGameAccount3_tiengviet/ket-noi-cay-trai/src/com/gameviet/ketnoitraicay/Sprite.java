...
import com.gameviet.ketnoitraicay.actor.Actor;
...
public class Sprite
{
	...
	public Sprite(String path, boolean isAurora, float scaleX, float scaleY)
	{
		if (activity == null || view == null)
			Log.e("Error : ", "Sprite Engine is not init. Please call initSprite(). please call it in onCreate(Bundle savedInstanceState)");
		...
		for (int i = 0; i < modulesLen; i++) {
			modules[i].image = Bitmap.createBitmap(modules[i].image, 0, 0, modules[i].width, modules[i].height, matrix, true);
			modules[i].width = (int) (modules[i].width * scaleX);
			modules[i].height = (int) (modules[i].height * scaleY);
		}
		//frame
		for (int i = 0; i < frameLen; i++) {
			for (int j = 0; j < frames[i].numofModule; j++) {
				frames[i].postionX[j] = (int) (frames[i].postionX[j] * scaleX);
				frames[i].postionY[j] = (int) (frames[i].postionY[j] * scaleY);
			}
		}
		//anim
		for (int i = 0; i < animLen; i++) {
			for (int j = 0; j < anims[i].numOfFrame; j++) {
				anims[i].postionX[j] = (int) (anims[i].postionX[j] * scaleX);
				anims[i].postionY[j] = (int) (anims[i].postionY[j] * scaleX);
			}
		}
	...
	private class Module
	{
		...
		public Module(int _id, int _type, int _color, int _x, int _y, int _w, int _h)
		{
			...
			if (type == 0)
				image = Bitmap.createBitmap(tileSheet, x, y, width, height, null, true);
		}
	...
	public void setAnim(Actor actor, int id, int x, int y, boolean loop, boolean showLastFrame)
	{
		actor._currentAnimation = id;
		actor._canLoop = loop;
		actor._ShowLastFrame = showLastFrame;
		actor._waitDelay = 0;
		actor._currentFrame = 0;
		actor._xpos = x;
		actor._ypos = y;
	...
	public void drawAnim(Canvas g, int angle, int Scalex, int Scaley, Actor actor, int x, int y)
	{
		...
		if (actor._currentAnimation >= 0) {
			...
			else if (actor._ShowLastFrame) {
				int frameIndex = anims[actor._currentAnimation].numOfFrame - 1;
			
				drawAFrame(g, anims[actor._currentAnimation].arrayFrameIndex[frameIndex], angle, Scalex, Scaley, x + anims[actor._currentAnimation].postionX[frameIndex], y + anims[actor._currentAnimation].postionY[frameIndex]);
	...
	public void drawAnim(Canvas g, Actor actor)
	{
		if (actor._currentAnimation >= 0) {
			if (actor._currentFrame < anims[actor._currentAnimation].numOfFrame)
				drawAFrame(g, anims[actor._currentAnimation].arrayFrameIndex[actor._currentFrame], actor._xpos + anims[actor._currentAnimation].postionX[actor._currentFrame], actor._ypos + anims[actor._currentAnimation].postionY[actor._currentFrame]);
			update(actor);
		}
	}
	...
	public static Bitmap loadImageFromAsset(String path)
	{
		...
		try {
	        ...
	        opt.inDither = true;   //important
   	        opt.inPreferredConfig = Bitmap.Config.ARGB_8888;
	       // return  BitmapFactory.decodeFile(path, opt);
			return BitmapFactory.decodeStream(am.open(path),null,opt);
			// buf.close();
		} catch (IOException e) {
			//
			e.printStackTrace(); 
