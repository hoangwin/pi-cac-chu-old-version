...
import com.xiaxio.twinscandy.state.StateWinLose;
...
public class Map
{
	...
	public void NewGame()
	{
		...
		countAutoSort = 1;
		mAllScore = 0;
		mTimerDecrease = 120000 - TwinsCandy.mcurrentlevel * 500;
		mTimerCount = 0;
		if (effect1 == null) {
			effect1 = new Actor();
			effect2 = new Actor();
	...
	public static void DrawPath(Canvas g)
	{
		...
		if (countFrameDrawPath-- >= 0) {
			...
			TwinsCandy.mainPaint.setStyle(Style.STROKE);
			TwinsCandy.mainPaint.setStrokeWidth(5);
			TwinsCandy.mainPaint.setColor(Color.BLUE);
			...
			for (i = 1; i < rCount; i++) {
				...
				g.drawLine(x1, y1, x2, y2, TwinsCandy.mainPaint); // Váº½ Ä‘Æ°á»�ng tháº³ng ná»‘i 2 Ã´
				x1 = x2;
				y1 = y2;
		...
		if (countFrameDrawAddScrore-- >= 0) {
			...
			//else
			TwinsCandy.fontbig_White.drawString(g, " + " + mScoreAdd, x1, y1 - (20 - countFrameDrawAddScrore * 2), BitmapFont.ALIGN_CENTER);
		}
	}
	...
	public static void DrawGame(Canvas g)
	{
		...
		TwinsCandy.mainPaint.setColor(Color.BLUE);
		int SquareFrame;
		if (TwinsCandy.mcurrentlevel < 15) {
			SquareFrame = 20;
		} else if (TwinsCandy.mcurrentlevel < 30) {
			SquareFrame = 22;
		} else if (TwinsCandy.mcurrentlevel < 45) {
			SquareFrame = 24;
		} else {
			SquareFrame = 26;
		}
