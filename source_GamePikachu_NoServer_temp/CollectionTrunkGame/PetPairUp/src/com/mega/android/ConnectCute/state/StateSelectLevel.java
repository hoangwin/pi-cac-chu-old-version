...
public class StateSelectLevel extends ConnectCute
{
	...
	public static synchronized void SendMessage(int type)
	{
		...
		case MESSAGE_CTOR: 
			...
			mcurrentPage = ConnectCute.mLevelUnlock / (NUM_ROW * NUM_COL);
		...
		case MESSAGE_UPDATE:
			for (int i = 0; i < NUM_ROW; i++) {
				...
				for (int j = 0; j < NUM_COL; j++) {
					...
					if (isTouchReleaseInRect(x - SELECTLEVEL_BUTTON_W / 2, y - SELECTLEVEL_BUTTON_H / 2, SELECTLEVEL_BUTTON_W, SELECTLEVEL_BUTTON_H)) {
						...
						ConnectCute.mcurrentlevel = mcurrentPage * NUM_COL * NUM_ROW + i * 5 + j;
						if (ConnectCute.mcurrentlevel <= ConnectCute.mLevelUnlock)
						{							
			...
			//left right button
			if (ConnectCute.isTouchReleaseInRect(SELECTLEVEL_BEGIN_X-  3*BUTTON_ARROW_CONFIRM_W/2, SCREEN_HEIGHT / 2, DEF.DIALOG_BUTTON_CONFIRM_W, DEF.DIALOG_BUTTON_CONFIRM_H)) {
				mcurrentPage--;
				if (mcurrentPage < 0)
			...
			//right	
			if (ConnectCute.isTouchReleaseInRect(SCREEN_WIDTH - (SELECTLEVEL_BEGIN_X-  BUTTON_ARROW_CONFIRM_W/2), SCREEN_HEIGHT / 2, DEF.DIALOG_BUTTON_CONFIRM_W, DEF.DIALOG_BUTTON_CONFIRM_H)) {
				mcurrentPage++;
				if (mcurrentPage >= MAX_PAG)
					mcurrentPage = 0;
				SoundManager.playSound(SoundManager.SOUND_SELECT, 1);
			...
			if (isKeyReleased(KeyEvent.KEYCODE_BACK) || ConnectCute.isTouchReleaseInRect(DEF.SELECTLEVEL_BACKGROUND_X , DEF.SELECTLEVEL_BACKGROUND_Y + 8, DEF.BUTTON_CANCEL_CONFIRM_W, DEF.BUTTON_CANCEL_CONFIRM_H))
			{
		...
		case MESSAGE_PAINT:
			if (!StateGameplay.isIngame) {
				...
				mainCanvas.drawRect(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, ConnectCute.mainPaint);
				//matrix.reset();
			...
			mainCanvas.drawText("Page : " + (mcurrentPage+1)+"/" +MAX_PAG,DEF.SELECTLEVEL_BACKGROUND_X +SELECTLEVEL_BUTTON_W+2 , DEF.SELECTLEVEL_BACKGROUND_Y-2, android_FontYellow);
			...
			for (int i = 0; i < NUM_ROW; i++) {
				...
				for (int j = 0; j < NUM_COL; j++) {
					x = SELECTLEVEL_BEGIN_X + j * (SELECTLEVEL_BUTTON_W + DEF.SELECTLEVEL_CONTENT_SPACE_W);
					ConnectCute.mcurrentlevel = mcurrentPage * NUM_COL * NUM_ROW + i * NUM_ROW + j;
					if (ConnectCute.mcurrentlevel <= ConnectCute.mLevelUnlock) {
						...
						//	fontbig_Yellow.drawString(mainCanvas, "Pass", x, y - fontbig_Yellow.getFontHeight()/2  , BitmapFont.ALIGN_CENTER);
							...
							mainCanvas.drawText("New", x, y + + (int)(80*scaleY), android_FontYellow);
					} else {
			...
			mainCanvas.drawText("SELECT LEVEL",SCREEN_WIDTH / 2,  (int)(60*scaleY), android_FontWhite);
			if(MAX_PAG>1)
			if (ConnectCute.isTouchDrapInRect(SELECTLEVEL_BEGIN_X -  3*BUTTON_ARROW_CONFIRM_W/2 , SCREEN_HEIGHT / 2, BUTTON_ARROW_CONFIRM_W, BUTTON_ARROW_CONFIRM_H))
				StateGameplay.spriteDPad.drawAFrame(ConnectCute.mainCanvas, DEF.FRAME_BUTTON_LEFT_HIGHTLIGHT,SELECTLEVEL_BEGIN_X-  3*BUTTON_ARROW_CONFIRM_W/2 , SCREEN_HEIGHT / 2);
			else
				StateGameplay.spriteDPad.drawAFrame(ConnectCute.mainCanvas, DEF.FRAME_BUTTON_LEFT_NORMAL,SELECTLEVEL_BEGIN_X -  3*BUTTON_ARROW_CONFIRM_W/2 , SCREEN_HEIGHT / 2);
			//right	
			if(MAX_PAG>1)
			if (ConnectCute.isTouchDrapInRect(SCREEN_WIDTH - (SELECTLEVEL_BEGIN_X-  BUTTON_ARROW_CONFIRM_W/2), SCREEN_HEIGHT / 2, BUTTON_ARROW_CONFIRM_W, BUTTON_ARROW_CONFIRM_H))
				StateGameplay.spriteDPad.drawAFrame(ConnectCute.mainCanvas, DEF.FRAME_BUTTON_RIGHT_HIGHTLIGHT, SCREEN_WIDTH - (SELECTLEVEL_BEGIN_X-  BUTTON_ARROW_CONFIRM_W/2), SCREEN_HEIGHT / 2);
			else
				StateGameplay.spriteDPad.drawAFrame(ConnectCute.mainCanvas, DEF.FRAME_BUTTON_RIGHT_NORMAL, SCREEN_WIDTH - (SELECTLEVEL_BEGIN_X-  BUTTON_ARROW_CONFIRM_W/2), SCREEN_HEIGHT / 2);

			//draw cancel
			if (ConnectCute.isTouchDrapInRect(DEF.SELECTLEVEL_BACKGROUND_X , DEF.SELECTLEVEL_BACKGROUND_Y + 8, DEF.BUTTON_CANCEL_CONFIRM_W, DEF.BUTTON_CANCEL_CONFIRM_H))
				StateGameplay.spriteDPad.drawAFrame(ConnectCute.mainCanvas, DEF.FRAME_CANCEL_HIGHTLIGHT, DEF.SELECTLEVEL_BACKGROUND_X , DEF.SELECTLEVEL_BACKGROUND_Y  + 8);
			else
				StateGameplay.spriteDPad.drawAFrame(ConnectCute.mainCanvas, DEF.FRAME_CANCEL_NORMAL, DEF.SELECTLEVEL_BACKGROUND_X , DEF.SELECTLEVEL_BACKGROUND_Y + 8);
