...
import caogia.android.pikachu.actor.Actor;
...
public class Map
{
	...
	// sá»‘ lÆ°á»£ng hÃ¬nh lÃ  15 => cÃ³ 60 tháº» hÃ¬nh, má»—i hÃ¬nh 4 tháº»
	final int CardNo = 16;
	...
	public void NewGame()
	{
		...
		countAutoSort = 1;
		mAllScore = 0;
		mTimerDecrease = 120000 - Pikachu.mcurrentlevel * 500;
		mTimerCount = 0;
		if (effect1 == null) {
			effect1 = new Actor();
			effect2 = new Actor();
	...
	public static void DrawPath(Canvas g)
	{
		...
		if (countFrameDrawPath-- >= 0) {
			...
			Pikachu.mainPaint.setStyle(Style.STROKE);
			Pikachu.mainPaint.setStrokeWidth(5);
			Pikachu.mainPaint.setColor(Color.BLUE);
			...
			for (i = 1; i < rCount; i++) {
				...
				g.drawLine(x1, y1, x2, y2, Pikachu.mainPaint); // Váº½ Ä‘Æ°á»�ng tháº³ng ná»‘i 2 Ã´
				x1 = x2;
				y1 = y2;
		...
		if (countFrameDrawAddScrore-- >= 0) {
			...
			//else
			Pikachu.fontbig_White.drawString(g, " + " + mScoreAdd, x1, y1 - (20 - countFrameDrawAddScrore * 2), BitmapFont.ALIGN_CENTER);
		}
	}
	...
	public static void DrawGame(Canvas g)
	{
		...
		Pikachu.mainPaint.setColor(Color.BLUE);
		int SquareFrame;
		if (Pikachu.mcurrentlevel < 15) {
			SquareFrame = 20;
		} else if (Pikachu.mcurrentlevel < 30) {
			SquareFrame = 22;
		} else if (Pikachu.mcurrentlevel < 45) {
			SquareFrame = 24;
		} else {
			SquareFrame = 26;
		}
