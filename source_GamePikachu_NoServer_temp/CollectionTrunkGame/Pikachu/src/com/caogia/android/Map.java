...
import com.caogia.android.actor.Actor;
import com.caogia.android.state.StateGameplay;
import com.caogia.android.state.StateWinLose;
...
public class Map
{
	...
	final int CardNo = 15;
	static int[][] CardMatrix; // mảng 2 chiều (8x12) lưu giá trị các thẻ hình
								// trong ma trận
	...
	static boolean CardSelected; // Thẻ hình thứ nhất đã được chọn
	...
	public void NewGame()
	{
		...
		countAutoSort = 1;
		mAllScore = 0;
		mTimerDecrease = 120000 - MainActivityHD.mcurrentlevel*500;
		mTimerCount = 0;
		if (effect1 == null) {
			effect1 = new Actor();
			effect2 = new Actor();
		...
		if (CardMatrix[y][x] != -1) // Nếu click vào một thẻ hình
		{
			...
			if (x != CardX1 || y != CardY1) {
				...
				if (CardMatrix[y][x] == CardMatrix[CardY1][CardX1]) {
					...
					rCount = Integer.MAX_VALUE; // ban đầu giả sử không có đường
												// đi
					tCount = 0;
					...
					if (rCount != Integer.MAX_VALUE) // tìm thấy đường đi rẽ
														// íthơn 2 lần
	...
	public static void DrawPath(Canvas g)
	{
		...
		if (countFrameDrawPath-- >= 0) {
			...
			MainActivityHD.mainPaint.setStyle(Style.STROKE);
			MainActivityHD.mainPaint.setStrokeWidth(5);
			MainActivityHD.mainPaint.setColor(Color.RED);
			...
			for (i = 1; i < rCount; i++) {
				...
				g.drawLine(x1, y1, x2, y2, MainActivityHD.mainPaint); // Vẽ đường thẳng nối 2 ô
				x1 = x2;
				y1 = y2;
		...
		if (countFrameDrawAddScrore-- >= 0) {
			...
			//else
			MainActivityHD.fontbig_White.drawString(g, " + " + mScoreAdd, x1, y1 - (20 - countFrameDrawAddScrore * 2), BitmapFont.ALIGN_CENTER);
		}
	}
	...
	public static void DrawGame(Canvas g)
	{
		...
		MainActivityHD.mainPaint.setStyle(Style.STROKE);
		MainActivityHD.mainPaint.setColor(Color.BLUE);
		int SquareFrame;
		if (MainActivityHD.mcurrentlevel < 15) {
			SquareFrame = 20;
		}
		else if (MainActivityHD.mcurrentlevel < 30) {
			SquareFrame = 22;
		} else if (MainActivityHD.mcurrentlevel < 45) {
			SquareFrame = 24;
		} else {
			SquareFrame = 26;
		}
