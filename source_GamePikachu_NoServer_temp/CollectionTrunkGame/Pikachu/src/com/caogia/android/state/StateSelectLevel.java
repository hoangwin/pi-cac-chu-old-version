...
public class StateSelectLevel extends MainActivityHD
{
	...
	public static synchronized void SendMessage(int type)
	{
		...
		case MESSAGE_CTOR: 
			...
			mcurrentPage = MainActivityHD.mLevelUnlock / (NUM_ROW * NUM_COL);
		...
		case MESSAGE_UPDATE:
			for (int i = 0; i < NUM_ROW; i++) {
				...
				for (int j = 0; j < NUM_COL; j++) {
					...
					if (isTouchReleaseInRect(x - SELECTLEVEL_BUTTON_W / 2, y - SELECTLEVEL_BUTTON_H / 2, SELECTLEVEL_BUTTON_W, SELECTLEVEL_BUTTON_H)) {
						...
						MainActivityHD.mcurrentlevel = mcurrentPage * NUM_COL * NUM_ROW + i * 5 + j;
						if (MainActivityHD.mcurrentlevel <= MainActivityHD.mLevelUnlock)
						{							
			...
			//left right button
			if (MainActivityHD.isTouchReleaseInRect(SELECTLEVEL_BEGIN_X-  3*BUTTON_ARROW_CONFIRM_W/2, SCREEN_HEIGHT / 2, DEF.DIALOG_BUTTON_CONFIRM_W, DEF.DIALOG_BUTTON_CONFIRM_H)) {
				mcurrentPage--;
				if (mcurrentPage < 0)
			...
			//right	
			if (MainActivityHD.isTouchReleaseInRect(SCREEN_WIDTH - (SELECTLEVEL_BEGIN_X-  BUTTON_ARROW_CONFIRM_W/2), SCREEN_HEIGHT / 2, DEF.DIALOG_BUTTON_CONFIRM_W, DEF.DIALOG_BUTTON_CONFIRM_H)) {
				mcurrentPage++;
			...
			if (isKeyReleased(KeyEvent.KEYCODE_BACK) || MainActivityHD.isTouchReleaseInRect(SCREEN_WIDTH - DEF.BUTTON_CANCEL_CONFIRM_W - 8 , DEF.SELECTLEVEL_BACKGROUND_Y + 8, DEF.BUTTON_CANCEL_CONFIRM_W, DEF.BUTTON_CANCEL_CONFIRM_H))
			{
		...
		case MESSAGE_PAINT:
			if (!StateGameplay.isIngame) {
				MainActivityHD.mainPaint.setColor(Color.BLACK);
				MainActivityHD.mainPaint.setStyle(Style.FILL);
				mainCanvas.drawRect(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, MainActivityHD.mainPaint);
				matrix.reset();
				matrix.setScale(SCREEN_WIDTH * 1f / 1280, SCREEN_HEIGHT * 1f / 800);
				mainPaint.setAntiAlias(true);
				mainPaint.setFilterBitmap(true);
				mainPaint.setDither(true);
				mainCanvas.drawBitmap(StateSelectLevel.backgroundImageSelectLevel, matrix, mainPaint);
			} else {
		...
		//	PikachuActivity.mainCanvas.drawRoundRect(rectF, 25, 25, PikachuActivity.mainPaint);//DEF.SELECTLEVEL_TITLE_CONTENT_SPACE_H
			...
			MainActivityHD.fontsmall_Yellow.drawString(MainActivityHD.mainCanvas, "Page : " + (mcurrentPage+1)+"/" +MAX_PAG,DEF.SELECTLEVEL_BACKGROUND_X +SELECTLEVEL_BUTTON_W+2 , DEF.SELECTLEVEL_BACKGROUND_Y-2, BitmapFont.ALIGN_LEFT);
			...
			if (MainActivityHD.isTouchDrapInRect(SELECTLEVEL_BEGIN_X -  3*BUTTON_ARROW_CONFIRM_W/2 , SCREEN_HEIGHT / 2, BUTTON_ARROW_CONFIRM_W, BUTTON_ARROW_CONFIRM_H))
				StateGameplay.spriteDPad.drawAFrame(MainActivityHD.mainCanvas, DEF.FRAME_BUTTON_LEFT_HIGHTLIGHT,SELECTLEVEL_BEGIN_X-  3*BUTTON_ARROW_CONFIRM_W/2 , SCREEN_HEIGHT / 2);
			else
				StateGameplay.spriteDPad.drawAFrame(MainActivityHD.mainCanvas, DEF.FRAME_BUTTON_LEFT_NORMAL,SELECTLEVEL_BEGIN_X -  3*BUTTON_ARROW_CONFIRM_W/2 , SCREEN_HEIGHT / 2);
			//right	
			if (MainActivityHD.isTouchDrapInRect(SCREEN_WIDTH - (SELECTLEVEL_BEGIN_X-  BUTTON_ARROW_CONFIRM_W/2), SCREEN_HEIGHT / 2, BUTTON_ARROW_CONFIRM_W, BUTTON_ARROW_CONFIRM_H))
				StateGameplay.spriteDPad.drawAFrame(MainActivityHD.mainCanvas, DEF.FRAME_BUTTON_RIGHT_HIGHTLIGHT, SCREEN_WIDTH - (SELECTLEVEL_BEGIN_X-  BUTTON_ARROW_CONFIRM_W/2), SCREEN_HEIGHT / 2);
			else
				StateGameplay.spriteDPad.drawAFrame(MainActivityHD.mainCanvas, DEF.FRAME_BUTTON_RIGHT_NORMAL, SCREEN_WIDTH - (SELECTLEVEL_BEGIN_X-  BUTTON_ARROW_CONFIRM_W/2), SCREEN_HEIGHT / 2);

			//draw cancel
			if (MainActivityHD.isTouchDrapInRect(SCREEN_WIDTH - DEF.BUTTON_CANCEL_CONFIRM_W - 8 , DEF.SELECTLEVEL_BACKGROUND_Y + 8, DEF.BUTTON_CANCEL_CONFIRM_W, DEF.BUTTON_CANCEL_CONFIRM_H))
				StateGameplay.spriteDPad.drawAFrame(MainActivityHD.mainCanvas, DEF.FRAME_CANCEL_HIGHTLIGHT, SCREEN_WIDTH - DEF.BUTTON_CANCEL_CONFIRM_W - 8  , DEF.SELECTLEVEL_BACKGROUND_Y  + 8);
			else
				StateGameplay.spriteDPad.drawAFrame(MainActivityHD.mainCanvas, DEF.FRAME_CANCEL_NORMAL, SCREEN_WIDTH - DEF.BUTTON_CANCEL_CONFIRM_W - 8  , DEF.SELECTLEVEL_BACKGROUND_Y + 8);
