...
public class Map
{
	...
	public void NewGame()
	{
		...
		countAutoSort = 1;
		mAllScore = 0;
		mTimerDecrease = 120000 - AnimalActivity.mcurrentlevel*500;
		mTimerCount = 0;
		if (effect1 == null) {
			effect1 = new Actor();
			effect2 = new Actor();
	...
	public static void DrawPath(Canvas g)
	{
		...
		if (countFrameDrawPath-- >= 0) {
			...
			AnimalActivity.mainPaint.setStyle(Style.STROKE);
			AnimalActivity.mainPaint.setStrokeWidth(5);
			AnimalActivity.mainPaint.setColor(Color.RED);
			...
			for (i = 1; i < rCount; i++) {
				...
				g.drawLine(x1, y1, x2, y2, AnimalActivity.mainPaint); // Vẽ đường thẳng nối 2 ô
				x1 = x2;
				y1 = y2;
		...
		if (countFrameDrawAddScrore-- >= 0) {
			...
			//else
			AnimalActivity.fontbig_White.drawString(g, " + " + mScoreAdd, x1, y1 - (20 - countFrameDrawAddScrore * 2), BitmapFont.ALIGN_CENTER);
		}
	}
	...
	public static void DrawGame(Canvas g)
	{
		...
		AnimalActivity.mainPaint.setStyle(Style.STROKE);
		AnimalActivity.mainPaint.setColor(Color.BLUE);
		for (int i = 0; i <= ROW + 1; i++)
			for (int j = 0; j <= COL + 1; j++) {
