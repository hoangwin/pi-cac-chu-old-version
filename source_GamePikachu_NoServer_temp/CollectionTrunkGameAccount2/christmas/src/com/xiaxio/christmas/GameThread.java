...
public class GameThread implements Runnable {
	...
	public void run() {
		while (true) {
			while (ChristmasActivity.running) {
				...
				if ((timeCurrent - timePrev) > 1000 / FRAME_RATE) {
					...
					ChristmasActivity.SendMessage(ChristmasActivity.mCurrentState, ChristmasActivity.MESSAGE_UPDATE);
					// MainActivity.SendMessage(MainActivity.mCurrentState,MainActivity.MESSAGE_PAINT);
					// hame ve se duoc chuyen xuong ben duoi

					ChristmasActivity.mainView.postInvalidate();
					ChristmasActivity.UpdateKey();
				//	Log.d("aaa", "aaaa");
					ChristmasActivity.UpdateTouch();
				} else {
					try {
						Thread.sleep(10);
