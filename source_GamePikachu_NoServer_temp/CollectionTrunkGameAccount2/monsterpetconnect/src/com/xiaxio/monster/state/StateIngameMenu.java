...
public class StateIngameMenu extends MonsterActivity implements IConstant
{
	...
	public static synchronized void SendMessage(int type)
	{
			...
			for (int i = 0; i < arrayMenu.length; i++) {
				...
				if (isTouchReleaseInRect(MENU_BEGIN_X - MENU_ELEMENT_W / 2, y - MENU_ELEMENT_H / 2, MENU_ELEMENT_W, MENU_ELEMENT_H)) {
					...
					case MENU_BACK_TO_MAINMENU:
						Dialog.showDialog(Dialog.DIALOG_TYPE_CONFIRM, "", "To Mainmenu?", Dialog.DIALOG_BACK_TO_MAINMENU_EXIT);
						break;
					case MENU_EXIT:
		...
		case MESSAGE_PAINT:
			...
			MonsterActivity.mainPaint.setARGB(150, 0, 0, 0);
			...
			MonsterActivity.mainPaint.setColor(Color.argb(180,0, 0, 0));
			MonsterActivity.mainCanvas.drawRect(0, 0, MonsterActivity.SCREEN_WIDTH, MonsterActivity.SCREEN_HEIGHT, MonsterActivity.mainPaint);
			...
			for (int i = 0; i < arrayMenu.length; i++) {
				...
				String tempstr = arrayMenuString[i];
				if(i==2)
				{
					if(isEnableSound)
						tempstr +="ON";
				...
				mainCanvas.drawText( tempstr, MENU_BEGIN_X, y + rect.height() / 2, android_FontNormal);
