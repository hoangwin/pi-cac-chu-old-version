...
public class Map
{
	...
	public void NewGame()
	{
		...
		countAutoSort = 1;
		mAllScore = 0;
		mTimerDecrease = 120000 - MonsterActivity.mcurrentlevel * 4000;
		mTimerCount = 0;
		if (effect1 == null) {
			effect1 = new Actor();
			effect2 = new Actor();
				...
				if (CardMatrix[y][x] == CardMatrix[CardY1][CardX1]) {
...

					{
						...
						for (int i = mcountAllPaired - 1; i >= 0; i--) {
							if ((GameViewThread.timeCurrent - timerPaired[i]) < (mcountAllPaired - i) * 1000) {
								mAllScoreBonus += 20;
								mScoreAdd += 20;
							}
						...
						mAllScore += mScoreAdd;
						timerPaired[mcountAllPaired++] = GameViewThread.timeCurrent;
	...
	public static void DrawPath(Canvas g)
	{
		...
		if (countFrameDrawPath-- >= 0) {
			...
			MonsterActivity.mainPaint.setStyle(Style.STROKE);
			MonsterActivity.mainPaint.setStrokeWidth(5);
			MonsterActivity.mainPaint.setColor(Color.BLUE);
			...
			for (i = 1; i < rCount; i++) {
				...
				g.drawLine(x1, y1, x2, y2, MonsterActivity.mainPaint); // Váº½ Ä‘Æ°á»�ng tháº³ng ná»‘i 2 Ã´
				x1 = x2;
				y1 = y2;
		...
		if (countFrameDrawAddScrore-- >= 0) {
			...
			//else
			MonsterActivity.fontbig_White.drawString(g, " + " + mScoreAdd, x1, y1 - (20 - countFrameDrawAddScrore * 2), BitmapFont.ALIGN_CENTER);
		}
	}
	...
	public static void DrawGame(Canvas g)
	{
		...
		MonsterActivity.mainPaint.setColor(Color.BLUE);
		int SquareFrame;
		if (MonsterActivity.mcurrentlevel < 15) {
			SquareFrame = 20;
		} else if (MonsterActivity.mcurrentlevel < 30) {
			SquareFrame = 22;
		} else if (MonsterActivity.mcurrentlevel < 45) {
			SquareFrame = 24;
		} else {
			SquareFrame = 26;
		}
			...
			for (int j = 0; j <= COL + 1; j++) {
				...
				} else // Náº¿u Ã´ hiá»‡n táº¡i khÃ´ng trá»‘ng
				{
					...
					if (CardSelected && CardX1 == j && CardY1 == i)
						StateGameplay.spriteTileBoard.drawAnim(g);
					MonsterActivity.mainPaint.setStyle(Style.STROKE);
