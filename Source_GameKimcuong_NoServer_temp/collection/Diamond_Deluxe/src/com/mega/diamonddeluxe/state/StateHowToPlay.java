...
public class StateHowToPlay extends Diamond implements IConstant
{
	...
	public static synchronized void SendMessage(int type)
	{
		...
		case MESSAGE_UPDATE:
			...
			if (isKeyReleased(KeyEvent.KEYCODE_BACK)|| Diamond.isTouchReleaseInRect(SCREEN_WIDTH-  DEF.BUTTON_CANCEL_CONFIRM_W - 8,0, DEF.BUTTON_CANCEL_CONFIRM_W, DEF.BUTTON_CANCEL_CONFIRM_H)) {
		...
		case MESSAGE_PAINT: 
			if (!StateGameplay.isIngame) {
				...
				//TapFruit.mainPaint.setColor(Color.BLACK);
				//TapFruit.mainPaint.setStyle(Style.FILL);
				//mainCanvas.drawRect(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT, TapFruit.mainPaint);
				matrix.reset();
				matrix.setScale(SCREEN_WIDTH * 1f / 800, SCREEN_HEIGHT * 1f / 1280);
				//mainCanvas.drawBitmap(StateMainMenu.splashBitmap, matrix, mainPaint);
		...
		// PikachuActivity.mainCanvas.drawRoundRect(rectF, 25, 25, PikachuActivity.mainPaint);//DEF.HOWTOPLAY_CONTENT_SPACE_H
			...
			if (Diamond.isTouchDrapInRect(SCREEN_WIDTH-  DEF.BUTTON_CANCEL_CONFIRM_W - 8, 0, DEF.BUTTON_CANCEL_CONFIRM_W, DEF.BUTTON_CANCEL_CONFIRM_H))
				StateGameplay.spriteDPad.drawAFrame(Diamond.mainCanvas, DEF.FRAME_CANCEL_HIGHTLIGHT, SCREEN_WIDTH-  DEF.BUTTON_CANCEL_CONFIRM_W - 8, 0);
			else
				StateGameplay.spriteDPad.drawAFrame(Diamond.mainCanvas, DEF.FRAME_CANCEL_NORMAL, SCREEN_WIDTH-  DEF.BUTTON_CANCEL_CONFIRM_W - 8, 0);
			break;
		case MESSAGE_DTOR:
			break;
		}
