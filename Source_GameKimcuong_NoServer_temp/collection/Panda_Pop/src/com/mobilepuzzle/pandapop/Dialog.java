...
import com.mobilepuzzle.pandapop.state.StateGameplay;
import com.mobilepuzzle.pandapop.state.StateMainMenu;
...
public class Dialog
{
	...
	public static int DIALOG_CONFIRM_Y = PandaPop.SCREEN_HEIGHT / 4;
	public static int DIALOG_CONFIRM_W = PandaPop.SCREEN_WIDTH - 2 * PandaPop.SCREEN_WIDTH /20;
	public static int DIALOG_CONFIRM_H = (int)(653*PandaPop.scaleY);
	...
	public static void drawDialog(Canvas c)
	{
		...
		PandaPop.spriteUI.drawAFrame(c, 0, DIALOG_CONFIRM_X, DIALOG_CONFIRM_Y);
		
		
		//draw text
		Rect textBounds =  new  Rect();
		PandaPop.android_FontNormal.getTextBounds("Maig", 0, "Maig".length(), textBounds);		
		PandaPop.mainCanvas.drawText( dialogText , PandaPop.SCREEN_WIDTH / 2,   DIALOG_CONFIRM_Y+ textBounds.height()*3 ,PandaPop.android_FontNormal);
		...
		int x2 =  PandaPop.SCREEN_WIDTH - DIALOG_CONFIRM_X - DIALOG_CONFIRM_W/4 - DEF.DIALOG_BUTTON_CONFIRM_W ;
		//draw button
		switch (dialogType)
		{
		case DIALOG_TYPE_CONFIRM:

			if (PandaPop.isTouchDrapInRect(x1, y , DEF.DIALOG_BUTTON_CONFIRM_W, DEF.DIALOG_BUTTON_CONFIRM_H))
				StateGameplay.spriteDPad.drawAFrame(PandaPop.mainCanvas, DEF.FRAME_CANCEL_HIGHTLIGHT, x1, y);
			else
				StateGameplay.spriteDPad.drawAFrame(PandaPop.mainCanvas, DEF.FRAME_CANCEL_NORMAL, x1, y);

			if (PandaPop.isTouchDrapInRect(x2, y , DEF.DIALOG_BUTTON_CONFIRM_W, DEF.DIALOG_BUTTON_CONFIRM_H))
			   
				StateGameplay.spriteDPad.drawAFrame(PandaPop.mainCanvas, DEF.FRAME_OK_HIGHTLIGHT, x2, y);
			else
				StateGameplay.spriteDPad.drawAFrame(PandaPop.mainCanvas, DEF.FRAME_OK_NORMAL, x2, y);
			break;
	...
	public static void updateDialog()
	{
		...
		case DIALOG_TYPE_CONFIRM:
			...
			if (PandaPop.isTouchReleaseInRect(x1 , y , DEF.DIALOG_BUTTON_CONFIRM_W, DEF.DIALOG_BUTTON_CONFIRM_H))//cancel
			{
				hideDialog();
			} else if (PandaPop.isTouchReleaseInRect(x2 , y , DEF.DIALOG_BUTTON_CONFIRM_W, DEF.DIALOG_BUTTON_CONFIRM_H))//ok
			{
				Dialog.hideDialog();
				...
				case DIALOG_NEXTSTATE_EXIT:
					...
					PandaPop.mainActivity.finish();
					break;
				case DIALOG_BACK_TO_MAINMENU_EXIT:
					PandaPop.mainActivity.saveGame();
					PandaPop.changeState(PandaPop.STATE_MAINMENU, true, true);
					break;
				case DIALOG_RESTART_GAME:
					PandaPop.changeState(PandaPop.STATE_GAMEPLAY, true, true);
				break;
				default:
					break;
				}
			...
			break;
		case DIALOG_TYPE_NOTICE:
			if (PandaPop.isTouchReleaseInRect(x2 , y , DEF.DIALOG_BUTTON_CONFIRM_W, DEF.DIALOG_BUTTON_CONFIRM_H))//ok
			{
				switch (dialogNextState)
				{
				case DIALOG_NEXTSTATE_CLOSE_LEADERBOARD_DIALOG:
					hideDialog();
					PandaPop.changeState(PandaPop.mPreState);
					break;
				//case DIALOG_BACK_TO_MAINMENU_EXIT:
				//	break;
