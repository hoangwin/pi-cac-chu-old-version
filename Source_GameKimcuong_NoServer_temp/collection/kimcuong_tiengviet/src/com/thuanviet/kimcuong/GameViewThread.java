			...
			while (Kimcuong.running) {
				...
				if ((timeCurrent - timePrev) > 1000 / FRAME_RATE_TARGET) {
					...
					if(GameLib.mainCanvas != null)
						Kimcuong.SendMessage(Kimcuong.mCurrentState,Kimcuong.MESSAGE_PAINT);
					if(GameLib.mainCanvas != null && holder != null)
					{
						holder.unlockCanvasAndPost(GameLib.mainCanvas);
						Kimcuong.SendMessage(Kimcuong.mCurrentState,Kimcuong.MESSAGE_UPDATE);
						Kimcuong.UpdateKey();					
						Kimcuong.UpdateTouch();
						timePrev2 = timePrev;
						timePrev = timeCurrent;
					}
