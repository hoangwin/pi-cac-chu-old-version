...
public class StateSelectLevel extends Monster
{
	...
	public static synchronized void SendMessage(int type)
	{
		...
		case MESSAGE_CTOR:
			...
			mcurrentPage = Monster.mLevelUnlock / (NUM_ROW * NUM_COL);
		...
		case MESSAGE_UPDATE:
			...
			for (int i = 0; i < NUM_ROW; i++) {
				...
				for (int j = 0; j < NUM_COL; j++) {
					...
					if (isTouchReleaseInRect(x - SELECTLEVEL_BUTTON_W / 2, y - SELECTLEVEL_BUTTON_H / 2, SELECTLEVEL_BUTTON_W, SELECTLEVEL_BUTTON_H)) {
						...
						Monster.mcurrentlevel = mcurrentPage * NUM_COL * NUM_ROW + i * NUM_COL + j;
			...
			//left right button
			if (MAX_PAG > 1)
				if (Monster.isTouchReleaseInRect(SELECTLEVEL_BEGIN_X - 3 * BUTTON_ARROW_CONFIRM_W / 2, SCREEN_HEIGHT / 2, DEF.DIALOG_BUTTON_CONFIRM_W, DEF.DIALOG_BUTTON_CONFIRM_H)) {
					mcurrentPage--;
					if (mcurrentPage < 0)
			...
			//right	
			if (MAX_PAG > 1)
				if (Monster.isTouchReleaseInRect(SCREEN_WIDTH - (SELECTLEVEL_BEGIN_X - BUTTON_ARROW_CONFIRM_W / 2), SCREEN_HEIGHT / 2, DEF.DIALOG_BUTTON_CONFIRM_W, DEF.DIALOG_BUTTON_CONFIRM_H)) {
					mcurrentPage++;
					if (mcurrentPage >= MAX_PAG)
						mcurrentPage = 0;
					SoundManager.playSound(SoundManager.SOUND_SELECT, 1);
			...
			if (isKeyReleased(KeyEvent.KEYCODE_BACK) || Monster.isTouchReleaseInRect(SCREEN_WIDTH-  DEF.BUTTON_CANCEL_CONFIRM_W - 8, DEF.SELECTLEVEL_BACKGROUND_Y + 8, DEF.BUTTON_CANCEL_CONFIRM_W, DEF.BUTTON_CANCEL_CONFIRM_H)) {
		...
		case MESSAGE_PAINT:
			...
			if (MAX_PAG > 1)
				if (Monster.isTouchDrapInRect(SELECTLEVEL_BEGIN_X - 3 * BUTTON_ARROW_CONFIRM_W / 2, SCREEN_HEIGHT / 2, BUTTON_ARROW_CONFIRM_W, BUTTON_ARROW_CONFIRM_H))
					StateGameplay.spriteDPad.drawAFrame(Monster.mainCanvas, DEF.FRAME_BUTTON_LEFT_HIGHTLIGHT, SELECTLEVEL_BEGIN_X - 3 * BUTTON_ARROW_CONFIRM_W / 2, SCREEN_HEIGHT / 2);
				else
					StateGameplay.spriteDPad.drawAFrame(Monster.mainCanvas, DEF.FRAME_BUTTON_LEFT_NORMAL, SELECTLEVEL_BEGIN_X - 3 * BUTTON_ARROW_CONFIRM_W / 2, SCREEN_HEIGHT / 2);
			//right	
			if (MAX_PAG > 1)
				if (Monster.isTouchDrapInRect(SCREEN_WIDTH - (SELECTLEVEL_BEGIN_X - BUTTON_ARROW_CONFIRM_W / 2), SCREEN_HEIGHT / 2, BUTTON_ARROW_CONFIRM_W, BUTTON_ARROW_CONFIRM_H))
					StateGameplay.spriteDPad.drawAFrame(Monster.mainCanvas, DEF.FRAME_BUTTON_RIGHT_HIGHTLIGHT, SCREEN_WIDTH - (SELECTLEVEL_BEGIN_X - BUTTON_ARROW_CONFIRM_W / 2), SCREEN_HEIGHT / 2);
				else
					StateGameplay.spriteDPad.drawAFrame(Monster.mainCanvas, DEF.FRAME_BUTTON_RIGHT_NORMAL, SCREEN_WIDTH - (SELECTLEVEL_BEGIN_X - BUTTON_ARROW_CONFIRM_W / 2), SCREEN_HEIGHT / 2);

			//draw cancel
			if (Monster.isTouchDrapInRect(SCREEN_WIDTH-  DEF.BUTTON_CANCEL_CONFIRM_W - 8, DEF.SELECTLEVEL_BACKGROUND_Y + 8, DEF.BUTTON_CANCEL_CONFIRM_W, DEF.BUTTON_CANCEL_CONFIRM_H))
				StateGameplay.spriteDPad.drawAFrame(Monster.mainCanvas, DEF.FRAME_CANCEL_HIGHTLIGHT, SCREEN_WIDTH-  DEF.BUTTON_CANCEL_CONFIRM_W - 8, DEF.SELECTLEVEL_BACKGROUND_Y + 8);
			else
				StateGameplay.spriteDPad.drawAFrame(Monster.mainCanvas, DEF.FRAME_CANCEL_NORMAL, SCREEN_WIDTH-  DEF.BUTTON_CANCEL_CONFIRM_W - 8, DEF.SELECTLEVEL_BACKGROUND_Y + 8);
