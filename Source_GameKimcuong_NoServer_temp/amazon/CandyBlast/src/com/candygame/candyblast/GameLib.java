...
public class GameLib extends Activity implements IConstant {
	...
	public static GameViewThread mainView;
	...
	public static Bitmap loadImageFromAsset(String path) {
		...
		try {
			...
			return BitmapFactory.decodeStream(am.open(path), null, options);
	...
	public static Bitmap loadImageNoALphaFromAsset(String path,int w,int h)
	{
		AssetManager am = mainGameLib.getAssets();
		// BufferedInputStream buf;
		// Bitmap bitmap = null;
		try {
			...
			Bitmap temp = BitmapFactory.decodeStream(am.open(path), null, options);
			Bitmap dest =  Bitmap.createScaledBitmap(temp, SCREEN_WIDTH, SCREEN_HEIGHT, false);
			temp.recycle();
			temp =null;
			System.gc();
			return dest;
	...
	public static int randomInt(int begin, int end) {
	 ...
	 @Override
	public boolean onTouchEvent(MotionEvent mo)  
	 {
		return false;
