...
public class GameViewThread extends SurfaceView implements Runnable {
	...
	public static int FRAME_RATE_TARGET = 20;
		...
		int miliSeccondInframe =  1000 / FRAME_RATE_TARGET;
			while (CandyPop.running) {
				...
				//FRAME_RATE_CURRENT = 1000/(timeCurrent - timePrev)
					...
					{
						CandyPop.SendMessage(CandyPop.mCurrentState,CandyPop.MESSAGE_PAINT);	holder.unlockCanvasAndPost(GameLib.mainCanvas);
						CandyPop.SendMessage(CandyPop.mCurrentState,CandyPop.MESSAGE_UPDATE);
						CandyPop.UpdateKey();					
						CandyPop.UpdateTouch();
						timePrev2 = timePrev;
						timePrev = timeCurrent;
						timeCurrent = System.currentTimeMillis();	
					}
					try {
						long detal = timeCurrent - timePrev;
						if(miliSeccondInframe - detal >0  )
						Thread.sleep(miliSeccondInframe - detal);
